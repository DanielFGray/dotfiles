...
vicious.register(memwidget, vicious.widgets.mem, '$1% ($2/$3)', 1)
...
vicious.register(netwupwidget, vicious.widgets.net,
    function (widget, args)
        if args["{wlan0 carrier}"] == 1 then
            return args["{wlan0 up_kb}"]..'<span color="'..theme.colors.base0..'">/</span>'..args["{wlan0 tx_mb}"]
        end
    end, 3)
...
vicious.register(netwdownwidget, vicious.widgets.net, 
    function (widget, args)
        if args["{wlan0 carrier}"] == 1 then
            return args["{wlan0 down_kb}"]..'<span color="'..theme.colors.base0..'">/</span>'..args["{wlan0 rx_mb}"]
        end
    end, 3)
...
for s = 1, screen.count() do
    ...
    mywibox[s][1].widgets = {
        ...
        mypromptbox[s],
        {
			...
			cpuwidget,cpuicon,
			spacer,
			netwupwidget,netupicon,
			space,
            netwdownwidget,netdownicon,
			wifiwidget,wifiicon,
            mpdwidget,mpdicon,
